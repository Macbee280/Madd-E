...
def show_boxes_and_regions(frame, boxes, masks):
    for i, box in enumerate(boxes):
        ...
        os.remove("cameraNoun1.txt")
        f = open("cameraNoun1.txt", "a")
        print(LABEL_MAP[cls-1], file=f, end='\r')
        f.close() 
        time.sleep(0.5)
        os.remove("cameraNoun2.txt")
        f = open("cameraNoun2.txt", "a")
        print(LABEL_MAP[cls-1], file=f, end='\r')
        f.close()
        time.sleep(0.5)
        os.remove("cameraNoun3.txt")
        f = open("cameraNoun3.txt", "a")
        print(LABEL_MAP[cls-1], file=f, end='\r')
        f.close() 
...
with dai.Device(pm.pipeline) as device:
    ...
    while True:
        ...
        if switch == 3:
            subprocess.run(["g++", "project.cpp"])
            time.sleep(2)
            subprocess.run(["./a.exe"])
            f = open('output.txt', 'r')
            content = f.read()
            audio = gTTS(text=content, lang="en", tld="co.uk")
            audio.save("tts.mp3")
            f.close()
